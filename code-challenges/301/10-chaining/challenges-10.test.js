...
//     let firstCount = curr.reduce((acc, cur) => {
//       if (cur === target) {
//         return acc + 1;
//       }
//       return acc;
//     }, 0);
//     return acc + firstCount;
//   }, 0);
//   return input;
// };
...
const count = (target, input) => {
  return input.reduce((accumulator, currentValue) => {
    const innerCount = currentValue.reduce((innerAccumulator, innerCurrentValue) => {
      if (innerCurrentValue === target) {
        return innerAccumulator + 1;
      }
      return innerAccumulator;
    }, 0);
    return accumulator + innerCount;
  }, 0);
};
...
//   input.reduce((acc, cur) => acc + cur.reduce((acc, cur) => acc + cur, 0), 0);
//   return input;
// };
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((element) => {
    return element.filter(item => typeof item === 'number' && item % 5 === 0).map(item => Math.pow(2, item));
  });
};
